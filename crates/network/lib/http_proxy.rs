...
async fn connect_via_proxy(
    ...
    // `target_host` is interpolated into the CONNECT request line. On the TLS
    // paths it derives from the guest's SNI, which is attacker-controlled, so a
    // CR/LF/space inside it would let a malicious guest inject extra header
    // lines (e.g. override `Proxy-Authorization`) or smuggle a second request
    // into the host's proxy connection. Reject any control char or whitespace
    // before it reaches the wire — fails closed, the connection never dials.
    if target_host
        .bytes()
...
pub(crate) mod test_support {
    ...
    /// Like [`spawn_connect_proxy`] but captures the **full** CONNECT request
    /// header block (so a test can assert headers such as `Proxy-Authorization`)
    /// and replies with `status_line` (e.g. `"HTTP/1.1 403 Forbidden"`). On a
    /// 2xx it splices to `origin`; otherwise it just closes after responding.
    pub(crate) async fn spawn_connect_proxy_ex(
        origin: SocketAddr,
        ...
        tokio::spawn(async move {
            ...
            if success {
                if let Ok(upstream) = TcpStream::connect(origin).await {
                    let (mut cr, mut cw) = client.into_split();
                    let (mut ur, mut uw) = upstream.into_split();
                    tokio::select! {
                        _ = tokio::io::copy(&mut cr, &mut uw) => {}
    ...
    /// Accept one connection, consume the CONNECT request, then close **without**
    /// responding — exercises the mid-handshake EOF path.
    pub(crate) async fn spawn_connect_proxy_eof() -> SocketAddr {
        let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
        let addr = listener.local_addr().unwrap();
        tokio::spawn(async move {
            if let Ok((mut client, _)) = listener.accept().await {
                ...
                // Drop `client` without sending a response.
            }
        });
        addr
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn connect_upstream_errors_on_non_2xx_connect_response() {
        use super::test_support::spawn_connect_proxy_ex;

        let origin = spawn_echo_origin().await;
        let (proxy_addr, _req) = spawn_connect_proxy_ex(origin, "HTTP/1.1 403 Forbidden").await;
        let cfg = ProxyConfig::from_url(&format!("http://{proxy_addr}")).unwrap();

        let dst: SocketAddr = "93.184.216.34:443".parse().unwrap();
        let err = connect_upstream(Some(&cfg), dst, Some("blocked.test"), true)
            .await
            .expect_err("a non-2xx CONNECT response must error");
        assert!(
            err.to_string().contains("refused CONNECT"),
            "unexpected error: {err}"
        );
    }

    #[tokio::test]
    async fn connect_upstream_sends_proxy_authorization_header() {
        use super::test_support::spawn_connect_proxy_ex;

        let origin = spawn_echo_origin().await;
        let (proxy_addr, req_rx) =
            spawn_connect_proxy_ex(origin, "HTTP/1.1 200 Connection established").await;
        // `user:pass@` userinfo becomes a Basic Proxy-Authorization header.
        let cfg = ProxyConfig::from_url(&format!("http://user:pass@{proxy_addr}")).unwrap();

        let dst: SocketAddr = "93.184.216.34:443".parse().unwrap();
        let _s = connect_upstream(Some(&cfg), dst, Some("origin.test"), true)
            .await
        ...
        assert!(
            req.contains(&format!("Proxy-Authorization: Basic {expected}\r\n")),
            "CONNECT request missing/malformed auth header: {req:?}"
        );
    }

    #[tokio::test]
    async fn connect_upstream_fails_closed_when_proxy_refused() {
        // Proxy configured but its port is closed; the dst is non-loopback so
        // the proxy applies. Must error rather than silently dialing direct —
        // a direct fallback would leak egress in a proxy-only network.
        let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
        let refused = listener.local_addr().unwrap();
        drop(listener);
        let cfg = ProxyConfig::from_url(&format!("http://{refused}")).unwrap();

        let dst: SocketAddr = "93.184.216.34:443".parse().unwrap();
        assert!(
            connect_upstream(Some(&cfg), dst, Some("origin.test"), true)
                .await
                .is_err(),
            "a configured-but-unreachable proxy must fail closed"
        );
    }

    #[tokio::test]
    async fn connect_upstream_rejects_crlf_injection_in_host() {
        // A guest-controlled SNI carrying embedded CRLF must never reach the
        // proxy CONNECT line. Rejected before any dial, so the proxy URL is a
        // throwaway that is never contacted.
        let cfg = ProxyConfig::from_url("http://127.0.0.1:1").unwrap();
        let dst: SocketAddr = "93.184.216.34:443".parse().unwrap();
        ...
        .await
        .expect_err("CRLF in the CONNECT host must be rejected");
        assert_eq!(err.kind(), io::ErrorKind::InvalidInput);
    }

    #[tokio::test]
    async fn connect_upstream_errors_on_proxy_eof_mid_handshake() {
        use super::test_support::spawn_connect_proxy_eof;

        let proxy_addr = spawn_connect_proxy_eof().await;
        let cfg = ProxyConfig::from_url(&format!("http://{proxy_addr}")).unwrap();

        let dst: SocketAddr = "93.184.216.34:443".parse().unwrap();
        let err = connect_upstream(Some(&cfg), dst, Some("origin.test"), true)
            .await
            .expect_err("a proxy closing mid-handshake must error");
